...
class Dataloader(pl.LightningDataModule):
    ...
    def setup(self, stage='fit'):
        if stage == 'fit':
            ...
            total_data = total_data.drop_duplicates(['sentence_1','sentence_2'], keep='first', ignore_index=True)
            ###############################################################
...
class Model(pl.LightningModule):
    ...
    def feature(self,inputs):
        ...
        last_hidden_states = outputs[0]  # 16,512,1024  --> (16,1024)
        feature = self.mean_pooling(outputs, inputs['attention_mask'])
        return feature

    def forward(self, x):
        feature = self.feature(x)
        outputs = self.fc(self.dropout(feature))
        return outputs.view(-1), feature
...
if __name__ == '__main__':
    ...
    for k in range(num_folds):
        ...
        output.to_csv(f'output-{k} fold scl.csv', index=False)
